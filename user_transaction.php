...
$id=$sid+1; 
$qrd="insert into mini(id,accno,taccno,amount,date,process)values('$id','$accno','$taccno','$amt','$date','Debit')";
  if ($conn->query($qrd) === TRUE)
   {
   }
   $id=$id+1;
   $qrd="insert into mini(id,accno,taccno,amount,date,process)values('$id','$taccno','$accno','$amt','$date','Credit')";
  if ($conn->query($qrd) === TRUE)
   {
   }
   //////////////////////
...
$amount2=$rw['balance'];
 $amt;
$amount11= $amt + $amount2;
	
	$qrt1="update user_register set balance='$amount11' where accno='$taccno'";
	if($conn->query($qrt1) === TRUE)
	  {
		 
		 echo "<script>alert('Your Transction is Success..');</script>";
	  }
...
$objEmail->message("http://localhost/Secure_banking_transaction_system/user_verification.php?email=".$email."&process=transaction&a=".$amt."&accno=".$accno."&taccno=".$taccno);
if ($objEmail->send())
{
...
/*
$wrt=$id.$accno.$name.$taccno.$amt.$date;
$r=md5($wrt);
$myfile = fopen("encrypt/trans".$nm.".txt", "w") or die("Unable to open file!");
			...
			<nav class="navbar navbar-default">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					<ul class="nav navbar-nav">
						...
						<li><a href="user_withdraw.php" class="scroll">Withdraw</a></li>    
						<li><a href="user_transaction.php" class="active">Transction</a></li>    
	...
	<div class="w3ls-section gallery">
		<div class="container">   
			<div class="w3ls-title">
				<h2 class="h3-w3l">Transaction</h2>
				<form name="form1" method="post" action="">
				  <table width="258" height="105" border="0" align="center">
                    <tr>
                      <td><span class="style4">Acc.No</span></td>
                      <td><input name="taccno" type="text" id="taccno" required="" /></td>
                    </tr>
                    <tr>
                      <td><span class="style4">Amount</span></td>
                      <td><input name="amt" type="text" id="amt" onKeyUp="enterNumber()" required=""/></td>
                    </tr>
                    <tr>
                      <td>Pin</td>
                    ...
                    <tr>
                      <td>&nbsp;</td>
                      <td><input name="btn" type="submit" id="btn" value="Transfer" /></td>
                    </tr>
                  </table>
				</form>
				<p class="h3-w3l">&nbsp;</p>
